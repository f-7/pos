...
<div class="box box-default">
			<div class="box-header with-border">
					<h3 class="box-title"> All product image list </h3>
			...
			<table id="product_info_table" class="table table-striped table-bordered responsive nowrap responsive_table" cellspacing="0" width="100%">
                    <thead>
                        <tr>
							<th>SL</th>
						    <th> Short note</th>
					...
					<tbody>
							...
							?>
								<tr>
									<td><?=$i++?></td>
									<td><?=$key->note?></td>
									<td><img style="width:270px; height:60px;" src="<?=base_url('website_file/product/'.$key->image)?>"/></td>
...
$(document).on("click",".action_button_edit",function(){
	...
	var save_link=base_url+"Web-application/product_update/";
	var obj=$(this);
...
$(document).on("click",".action_button_delete",function(){
	...
	var save_link=base_url+"Web-application/product_delete/";
	  var  rows = $(this).closest("tr");
	   var table = $('#product_info_table').DataTable(); 
...
if (result) {
	 ...
	 if(view=="done")
	 {
		 alert("Successfully delete this Image!");	
