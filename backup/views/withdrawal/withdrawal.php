...
<!-- issue_date account_number loan_amount deposit_amount installment_type installment_amount installment_duration status remark created_dateapproval -->
	              ...
	              <!--      <div class="form-group">
	                     ...
	                     <div class="form-group">
	                      ...
	                      <div class="col-sm-8">
	                        <input readonly="true" type="text" class="form-control" name='withdrawal_amount' id="withdrawal_amount"  required="true" value=" <?php echo isset($info['loan_info']->number_of_istallment)?$info['loan_info']->number_of_istallment:"" ;?>">
	                      </div>
	                    </div>
...
<script type="text/javascript">
	...
	function getAccountDepositBalance( account_number){
		...
		$.post(base_url+"ajax-query/getAccountDepositBalance",{account_number:account_number},function(data){
			...
			$("#deposit_amount").val(data.collection_deposit_amount - data.withdrawal_amount);	

			var allowable_amt = (((data.loan_amount - data.collection_loan_amount)*10)/100);

			if(allowable_amt < data.collection_deposit_amount && data.collection_deposit_amount >= data.withdrawal_amount){ 
				$("#withdrawal_amount").removeAttr('readonly');
			}
			$("#allowable_withdrawal_amt").val(data.collection_deposit_amount - allowable_amt - data.withdrawal_amount);	
