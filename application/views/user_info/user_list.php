 ...
 <script>
  ...
  $(document).ready(function() {
    ...
    $('#user_list_table').DataTable( {
        ...
        "ajax": {
			 scriptCharset: "utf-8",
            "url": base_url+"user-information/user-list-get",
            "type": "POST",
			"dataType": "json",
        },
        "columns": [
			...
			{ data: null, render: function ( data, type, row ) {   
				...
				var delete_button=(data.delete_oparation=='true')?'<a  rel="'+id+'"  class="action_button action_button_delete">Delete</a>':'';
				
				var user_update_link=base_url+"user-information/user-update?id="+id;
				if(data.currrent_user_type==1){
				var button_list='<a  rel="'+id+'" onclick="viewUser('+data.user_id+',this)"  class="action_button action_button_view">View</a><a href="'+user_update_link+'" target="_blank" rel="'+id+'" class="action_button action_button_edit">Edit</a>'+delete_button;
				}else{
 ...
 $(document).on("click",".action_button_delete",function(){
	  ...
	  var  rows = $(this).closest("tr");
	 	...
	 	$.confirm({
			...
			'message'	: 'You are about to delete this user. <br />It\'s cannot be restored at a later time! <br> <b style="color:red"> Continue ?</b>',
			'buttons'	: {
				'Yes'	: {
					'class'	: 'blue',
					'action': function(){
						 ...
						 $.post(base_url+"user-information/update-user-status",{id:user_id,status:0},function(view){
							 		if(view=="done")
									{
