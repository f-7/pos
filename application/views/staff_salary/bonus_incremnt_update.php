				 ...
				 <div class="col-md-7 product_category_info_info_col_1">                  
					...
					<?php 
 						foreach($data_list as $skey){ ?>
						  <option value="<?=$skey->id?>"  <?=($bonus_data->user_id==$skey->id)?"selected":""?>><?=$skey->name?></option>
						<?php
 						}
						?>
					 ...
					 <div class="micro-pos-group">
                      ...
                      <div class="col-sm-7"> 
					   <select class="form-control month_name" required="required" name="month_name">
                         <option value="">Select</option>
						  <?=OOP::selectOptionList(OOP::getMonth(),$bonus_data->month_name)?>
                      </select>
...
?>				 
				  ...
				  <div class="micro-pos-group">
                      ...
                      <div class="col-sm-7">
					   ...
					   <select name="year"   class="form-control year" required="required">
						...
						<option value="<?=$year_no?>" <?=($bonus_data->year==$year_no)?"selected":""?>><?=$year_no?></option>
						<option value="<?=$year_no-1?>" <?=($bonus_data->year==$year_no)?"selected":""?>><?=$year_no-1?></option>
						<option value="<?=$year_no-2?>" <?=($bonus_data->year==$year_no)?"selected":""?>><?=$year_no-2?></option> 
						<option value="<?=$year_no-3?>" <?=($bonus_data->year==$year_no)?"selected":""?>><?=$year_no-3?></option> 
						<option value="<?=$year_no-4?>" <?=($bonus_data->year==$year_no)?"selected":""?>><?=$year_no-4?></option> 
					   <option value="<?=$year_no-5?>" <?=($bonus_data->year==$year_no)?"selected":""?>><?=$year_no-5?></option> 
				 ...
				 <div class="col-md-5">  
					 ...
					 <div class="micro-pos-group">
                      ...
                      <div class="col-sm-7">
					   ...
					   <select class="form-control type" required="required" name="type">
                         <option value="">Select</option>
						  <?=OOP::selectOptionList(OOP::getSalaryType(),$bonus_data->type)?>
                      </select>
					 <errormessage> <?=(form_error('type'))?form_error('type'):""?> </errormessage>
					  ...
					  <input type="text" name="amount" value="<?=$bonus_data->amount?>" class="form-control amount" required="required"  placeholder="Amount">
				 	 ...
				 	 <input type="hidden" name="bonus_data_id" value="<?=$bonus_data->id?>" />
