...
{	
	function __construct(){
     ...
     public function getUserIdFromDistributionID($loan_distribution_id){
...
loan_distribution.loan_amount,loan_distribution.deposit_amount,loan_distribution.issue_date,loan_distribution.installment_amount,staff.name staff_name');
         ...
         public function getMonthlyCollection($collection_date){

          
            $sql = "SELECT loan_transaction.pay_date,sum(loan_transaction.collection_loan_amount) collection_loan_amount, sum(loan_transaction.collection_deposit_amount) collection_deposit_amount, area.area_name, area.area_code
FROM `loan_distribution`
...
GROUP BY  day(loan_transaction.pay_date),area.area_code  order by area.area_code,pay_date";  

            $query = $this->db->query($sql,array($collection_date,$collection_date));
           // echo $this->db->last_query(); exit();
            if($query->num_rows() > 0) {
                return $query->result();
            }else{
...
loan_distribution.account_number,loan_distribution.loan_amount,loan_distribution.deposit_amount,loan_distribution.issue_date,loan_distribution.installment_type,loan_distribution.installment_duration,loan_distribution.number_of_istallment,loan_distribution.status,loan_distribution.approved_by,loan_distribution.installment_amount');
        ...
        public function loanDistributionReport($collection_date=null, $area=null,$society=null,$report_type=null){
            ...
            $this->db->select("stock_info.buy_unit_price,users_profiles.name,users_profiles.fathers_name,users_profiles.spouse_name,account.account_number,society.society_code,society.society_name,area.area_code,area.area_name,loan_distribution.issue_date,
loan_distribution.loan_amount,loan_distribution.deposit_amount,sell_details.serial_no,sell_details.invoice_id,up.name staff,ref.name ref,product_name_list.product_name");
             $this->db->from('loan_distribution');
                ...
                $this->db->join('area','area.area_code=society.area_code','inner');   
                 $this->db->join('staff_working_schedule','staff_working_schedule.society_code = society.society_code and staff_working_schedule.status=1','inner');   
                  $this->db->join('users_profiles up','up.user_id = staff_working_schedule.user_id','inner');  
                   $this->db->join('users_profiles ref','ref.user_id = loan_details.user_id','inner');  
        ...
        $this->db->order_by('area.area_code',"asc");
         ...
         public function loanDistributionReportReffence($collection_date=null, $area=null,$society=null,$report_type=null){
            ...
            $this->db->join('product_name_list','product_name_list.id=product_info.product_name_id','inner');
             ...
             $this->db->join('account','loan_distribution.account_number=account.account_number','inner');   
              $this->db->join('users_profiles','users_profiles.user_id=account.user_id','inner');   
               $this->db->join('society','society.society_code=users_profiles.society_code','inner');   
                $this->db->join('area','area.area_code=society.area_code','inner');                 
                  $this->db->join('users_profiles up','up.user_id = loan_details.user_id','inner');   
...
GROUP BY `loan_distribution`.`id`";  
 ...
 `society`.`society_code`, `society`.`society_name`, `loan_distribution`.`loan_amount`, `loan_distribution`.`deposit_amount`
FROM `loan_distribution`
left JOIN `loan_transaction` ON `loan_distribution`.`id`=`loan_transaction`.`loan_distribution_id`
INNER JOIN `account` ON `account`.`account_number`=`loan_distribution`.`account_number`
INNER JOIN `users_profiles` ON `users_profiles`.`user_id`=`account`.`user_id`
INNER JOIN `society` ON `society`.`society_code`=`users_profiles`.`society_code`
INNER JOIN `area` ON `area`.`area_code`=`society`.`area_code`
WHERE `loan_distribution`.`status` >0 AND `loan_distribution`.`status` <3  and `account`.`account_number` = ?
GROUP BY `loan_distribution`.`id` HAVING loan_distribution.loan_amount >= sum( if(loan_transaction.collection_loan_amount is null,0,loan_transaction.collection_loan_amount))";  
...
`area`.`area_name`, `society`.`society_code`, `society`.`society_name`, sum(loan_transaction.collection_loan_amount) collection_loan_amount,
 sum(loan_transaction.collection_deposit_amount) collection_deposit_amount, users_profiles.user_id
FROM `loan_distribution` 
left JOIN `loan_transaction` ON `loan_distribution`.`id`=`loan_transaction`.`loan_distribution_id` 
INNER JOIN `account` ON `account`.`account_number`=`loan_distribution`.`account_number` 
INNER JOIN `users_profiles` ON `users_profiles`.`user_id`=`account`.`user_id` 
...
sum(loan_transaction.collection_deposit_amount) collection_deposit_amount');
        ...
        public function getLoanChartData($date = null){
         ...
         public function getCurrenMonthCollectionProgress($current_date){
            ...
            sum(deposit_amount) deposit_amount,sum(collection_deposit_amount) collection_deposit_amount  from (select area.area_code,area.area_name,sum(if(loan_distribution.installment_type=0,loan_distribution.installment_amount*5,loan_distribution.installment_amount)) installment_amount, 0 collection_loan_amount,
            sum(if(loan_distribution.installment_type=0,loan_distribution.deposit_amount*5,loan_distribution.deposit_amount)) deposit_amount,0 collection_deposit_amount
 from loan_distribution 
inner join account on account.account_number = loan_distribution.account_number
inner join users_profiles on users_profiles.user_id = account.user_id
inner join `area` on area.area_code = users_profiles.area_no
where loan_distribution.status = 1
...
group by users_profiles.area_no) t group by t.area_code";  
            ...
            $query = $this->db->query($sql,array($current_date,$current_date));
            //echo $this->db->last_query(); exit();
            if($query->num_rows() > 0) {
                return $query->result();
            }else{
...
`area`.`area_code`, `area`.`area_name`, `society`.`society_code`, `society`.`society_name`, sum(loan_transaction.collection_loan_amount) collection_loan_amount, 
sum(loan_transaction.collection_deposit_amount) collection_deposit_amount,0 txn_coll_loan,0 txn_coll_deposit,0 txn_id 
FROM `loan_distribution` left JOIN `loan_transaction` ON `loan_distribution`.`id`=`loan_transaction`.`loan_distribution_id` 
INNER JOIN `account` ON `account`.`account_number`=`loan_distribution`.`account_number` 
INNER JOIN `users_profiles` ON `users_profiles`.`user_id`=`account`.`user_id` 
INNER JOIN `society` ON `society`.`society_code`=`users_profiles`.`society_code` 
INNER JOIN `area` ON `area`.`area_code`=`society`.`area_code` 
...
and staff_working_schedule.working_day = ? and area.area_code = ? GROUP BY `loan_distribution`.`id`";
        ...
        public function getLoanDitributionMonthly($current_date){

            
            $sql = "select society.society_name,area.area_code,area.area_name,sum(loan_distribution.loan_amount) loan_amount
 from loan_distribution 
inner join account on account.account_number = loan_distribution.account_number
inner join users_profiles on users_profiles.user_id = account.user_id
inner join society on society.society_code = users_profiles.society_code
inner join `area` on area.area_code = users_profiles.area_no
where loan_distribution.status > 0 and month(loan_distribution.issue_date) = month(?) and year(loan_distribution.issue_date) = year(?)    
group by users_profiles.society_code";  
         ...
         public function getLoanStayAreaWise(){

            
            $sql = "select t.area_code,t.area_name,sum(loan_amount) loan_amount, sum(collection_loan_amount) collection_loan_amount,sum(loan_amount) - sum(collection_loan_amount) stay
              from (
select area.area_code,area.area_name,sum(loan_distribution.loan_amount) loan_amount, 0 collection_loan_amount
 from loan_distribution 
inner join account on account.account_number = loan_distribution.account_number
inner join users_profiles on users_profiles.user_id = account.user_id
inner join `area` on area.area_code = users_profiles.area_no
...
union all 
select area.area_code,area.area_name,0 loan_amount, sum(loan_transaction.collection_loan_amount) collection_loan_amount
...
group by users_profiles.area_no) t group by t.area_code";  
        ...
        public function getReferenceInfo($loan_distribution_id){


            $this->db->select('users_profiles.name');

            $this->db->from('loan_details');
             $this->db->join('users_profiles','loan_details.user_id = users_profiles.user_id','inner');
             $this->db->where('loan_details.ref_id ',$loan_distribution_id);  
