...
class Tank_auth
{
		...
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			//	if ( md5("$$@".$password)== $user->password) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						$user_prof=$this->ci->users->get_user_profile_by_id($user->id);
						
						$this->ci->session->set_userdata(array(
								'user_id'	=> $user->id,
								'username'	=> $user->username,		
								'type'	=> $user->type,
								'member_photo'=>$user_prof->member_photo,
								'name'=>$user_prof->name,	
								'reference_number'=>$user->reference_pin_n,								
								'reference_number'=>$user->reference_pin_number,	
	...
	function create_repassword($password)
	{
